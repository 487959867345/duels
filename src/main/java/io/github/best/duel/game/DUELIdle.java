...
public class DUELIdle {
    ...
    public void onOpen(long time, DUELConfig config) {
    ...
    private void tickStartWaiting(long time, GameWorld world) {
        ...
        if (sec_f > 1) {
            for (ServerPlayerEntity player : world.getPlayers()) {
                ...
                player.teleport(state.lastPos.x, 65, state.lastPos.z);
            }
        }
        ...
        if ((this.startTime - time) % 20 == 0) {
            if (sec > 0) {
                DUELActive.broadcastTitle(new LiteralText(Integer.toString(sec)).formatted(Formatting.BOLD), world);
                DUELActive.broadcastSound(SoundEvents.BLOCK_NOTE_BLOCK_HARP, 1.0F, world);
            } else {
                DUELActive.broadcastTitle(new LiteralText("Go!").formatted(Formatting.BOLD), world);
                DUELActive.broadcastSound(SoundEvents.BLOCK_NOTE_BLOCK_HARP, 2.0F, world);
            }
        }
    }
